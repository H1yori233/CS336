    ...
    def _apply_bpe(self, tokens: list[bytes]) -> list[bytes]:
        ...
        while True:
            ...
            if not pairs:
                break
            best_pair = min(
                pairs, key=lambda pair: self.merge_ranks.get(pair, float("inf"))
            )
...
def train_and_profile_cli():
    ...
    parser.add_argument(
        "--vocab_size", type=int, default=4096, help="Desired vocabulary size."
    )
    parser.add_argument(
        "--special_tokens",
        type=str,
    ...
    print(
        f"   Found {len(tokenizer.pretoken_counts)} unique pre-tokenized words/chunks."
    )
    ...
    print(
        f"   Learned {len(tokenizer.merges)} merges. Final vocab size: {len(tokenizer.vocab)}"
    )
